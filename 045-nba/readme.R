...
court_points <- tbl_df(court_points)
court_points <- court_points %>% 
  mutate(x = round(x, 3),
         y = round(y, 3))

colcour <- "#BF802E"
bkgimg <- "https://s-media-cache-ak0.pinimg.com/736x/d9/53/f0/d953f00e555f7d5a628603e9bbe35f71.jpg"
size <- 350
hc <- highchart(width = size, height = size) %>% 
  hc_add_theme(hc_theme(chart  = list(backgroundColor = colcour, divBackgroundImage  = NULL))) %>% 
...
for (d in unique(court_points$desc)) {
  message(d)
  daux <- court_points %>% filter(desc == d) 
  hc <- hc %>% hc_add_serie(data = daux %>% select(x, y) %>% list.parse2(),
                            name = d)
}
hc
